...
void Statistics(const vector<Timetable>& analysis, int days) 
{
    ...
    for (const auto& t : analysis)
    {
        ...
        statMap[t.no].seatOccupancyRate += double(t.GetSold()) / double(t.GetMax()) * 100.0;
    }

    for (const auto& p : statMap) {
        const string& no = p.first;
        const Stats& s = p.second;
        const double& sor = p.second.seatOccupancyRate;
        double avgSold = (double)s.totalSold / days;
        double avgIncome = (double)s.totalIncome / days;
        cout << "车次：" << no << "\n总售票数：" << s.totalSold << "\n总收入：" << s.totalIncome << "\n日均乘客数："
            << fixed << setprecision(2) << avgSold << "\n日均收入：" << fixed << setprecision(2) << avgIncome
            << "\n上座率：" << fixed << setprecision(2) << sor / days << "%" << endl;
    }
}